...
import { API_BASE_URL, IMAGE_BASE_URL, httpClient } from '../constants/Api';
...
function Brands() {
  ...
  const getBrands = async () => {
    try {
      // const getBrandList = await axios.get('http://107.20.14.162:8003/api/auth/all-brands');
      // const getBrandList = await axios.get(API_BASE_URL + BRANDS.GET_BRANDS);
      const getBrandList = await httpClient.get(API_BASE_URL + BRANDS.GET_BRANDS);
      console.log({getBrandList})
      if (getBrandList.status === 200) {
        setBrands(getBrandList.data.data.company_brands);
        // toast.success(getBrandList.data.message);
      }
    } catch (err) {
      console.log(err);
    }
